...
#pragma config(Sensor, S2,     lultra,                sensorSONAR)
...
int mSD, mSE, mSF, mSG;
	  ...
	  while(SensorValue(lultra) > 100 || SensorValue(fultra) > 100){
	     runMotorSpeeds(mSD, mSE, mSF, mSG, 213, 25);
	  }
	  stopMotors();
	...
	if(nMessage[0] == 2){
	    ...
	    while(SensorValue(ultra) > 100 || SensorValue(fultra) > 100){
	      runMotorSpeeds(mSD, mSE, mSF, mSG, 323, 25);
	  }
	  stopMotors();
	...
	if(nMessage[0] == 3){
	   ...
	   while(SensorValue(fultra) < 250) {

	  runMotorSpeeds(mSD, mSE, mSF, mSG, 270, -25);
	}

	while(SensorValue(fultra) < 275) {
	...
	while(SensorValue(fultra) > 50) {
	  ...
	  runMotorSpeeds(mSD, mSE, mSF, mSG, 180, 25);
	}
