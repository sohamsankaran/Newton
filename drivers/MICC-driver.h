...
/*
 * $Id: MICC-driver.h 63 2011-08-20 15:27:52Z xander $
 */
 ...
 *
 * MICC-driver.h provides an API for the MicroInfinity CruizCore XG1300L sensor.
 *
 * Changelog:
 * - 0.1: Initial release
 *
 ...
 * \date 29 May 2011
 * \version 0.1
 * \example MICC-test1.c
 * \example MICC-test2.c
 */
...
int MICCreadRelativeHeading(tSensors link) {
  ...
  if (!writeI2C(link, MICC_I2CRequest, 2))
    return 0;

  if (!readI2C(link, MICC_I2CReply, 2))
    return 0;
...
bool MICCreadAccel(tSensors link, int &x_accel, int &y_accel, int &z_accel) {
  ...
  if (!writeI2C(link, MICC_I2CRequest, 6))
    return false;

  if (!readI2C(link, MICC_I2CReply, 6))
    return false;
...
bool MICCsendCmd(tSensors link, ubyte command) {
  ...
  return writeI2C(link, MICC_I2CRequest, 0);
}
